...
class decoder(nn.Module):
    def __init__(self):
        # input: [16, 448, 12, 12]
        self.decoder = nn.Sequential(nn.Conv2d(in_channels=448, out_channels=160, kernel_size=2, padding=0, stride=2), nn.BatchNorm2d(num_features=160), nn.SiLU(),  # [16, 160, 24, 24]
                                     nn.Conv2d(in_channels=160, out_channels=160, kernel_size=5, padding=2, stride=1), nn.BatchNorm2d(num_features=160), nn.SiLU(),  # [16, 160, 24, 24]

                                     nn.Conv2d(in_channels=160, out_channels=56, kernel_size=2, padding=0, stride=2), nn.BatchNorm2d(num_features=56), nn.SiLU(),  # [16, 56, 48, 48]
                                     nn.Conv2d(in_channels=56, out_channels=56, kernel_size=5, padding=2, stride=1), nn.BatchNorm2d(num_features=56), nn.SiLU(),  # [16, 56, 48, 48]

                                     nn.Conv2d(in_channels=56, out_channels=32, kernel_size=2, padding=0, stride=2), nn.BatchNorm2d(num_features=32), nn.SiLU(),  # [16, 32, 96, 96]
                                     nn.Conv2d(in_channels=32, out_channels=32, kernel_size=5, padding=2, stride=1), nn.BatchNorm2d(num_features=32), nn.SiLU(),  # [16, 32, 96, 96]

                                     nn.Conv2d(in_channels=32, out_channels=24, kernel_size=2, padding=0, stride=2), nn.BatchNorm2d(num_features=24), nn.SiLU(),  # [16, 24, 192, 192]
                                     nn.Conv2d(in_channels=24, out_channels=24, kernel_size=5, padding=2, stride=1), nn.BatchNorm2d(num_features=24), nn.SiLU(),  # [16, 24, 192, 192]

                                     nn.Conv2d(in_channels=24, out_channels=3, kernel_size=33, padding=0, stride=1), nn.BatchNorm2d(num_features=3), nn.SiLU(),  # [16, 3, 224, 224]
                                     nn.Conv2d(in_channels=3, out_channels=3, kernel_size=5, padding=2, stride=1), nn.Sigmoid(),  # [16, 3, 224, 224]
...
class efficientnet_b4(nn.Module):
    def __init__(self, args):
        ...
        if self.choice == 1:
            self.model = timm.create_model('tf_efficientnet_b4' + self.init_model_extension, pretrained=True)
            print("Loaded weights from : ", 'tf_efficientnet_b4' + self.init_model_extension)
        else:
            self.model = timm.create_model('tf_efficientnet_b4' + self.init_model_extension, pretrained=False)
        ...
        if self.use_self_attn:
            ...
            if '2' in self.self_attn_layers:
                self.sa_2 = SelfAttention(in_dim=56)
            if '3' in self.self_attn_layers:
                self.sa_3 = SelfAttention(in_dim=112)
            if '4' in self.self_attn_layers:
                self.sa_4 = SelfAttention(in_dim=160)
            if '5' in self.self_attn_layers:
                self.sa_5 = SelfAttention(in_dim=272)
            if '6' in self.self_attn_layers:
                self.sa_6 = SelfAttention(in_dim=448)
    ...
    def forward(self, args, x, label, cat_label=None, enable_mixup=False, training=False):
        ...
        # i: 2    output.size(): torch.Size([16, 56, 48, 48])
        # i: 3    output.size(): torch.Size([16, 112, 24, 24])
        # i: 4    output.size(): torch.Size([16, 160, 24, 24])
        # i: 5    output.size(): torch.Size([16, 272, 12, 12])
        # i: 6    output.size(): torch.Size([16, 448, 12, 12])
        ...
        output = F.dropout(output, p=self.dropout_prob, training=training, inplace=True)
