...
class efficientnet_b3(nn.Module):
    def __init__(self, args):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels=3, out_channels=3, kernel_size=(10, 10), padding=4, stride=2),
                                   nn.BatchNorm2d(num_features=3), Swish())
        self.conv2 = nn.Sequential(nn.Conv2d(in_channels=3, out_channels=3, kernel_size=(11, 11), padding=5, stride=1),
                                   nn.BatchNorm2d(num_features=3), Swish())
        self.conv3 = nn.Sequential(nn.Conv2d(in_channels=3, out_channels=3, kernel_size=(11, 11), padding=5, stride=1),
                                   nn.BatchNorm2d(num_features=3), Swish())
    ...
    def forward(self, args, x, label, enable_mixup=False):
        ...
        output = self.conv1(output)  # 1024 -> 512
        output = self.conv2(output)  # 512 -> 512
        output = self.conv3(output)  # 512 -> 512
